...
% Title
\title{What's in Your Transit? \\ Towards Reliably Getting More Science from Exoplanet Transit Data}
...
Here we characterize this shortfall (the Transit-Depth Precision Problem), by defining an amplification factor, $A$, quantifying the discrepancy between measured transit-depth uncertainty and baseline scatter for a similar time bin. 
While in theory $A$ should be $\sim\sqrt{3}$, we find that it departs substantially from this limit due to correlations between $D$ and the limb-darkening coefficients (LDCs). We show that (1) transit-based science can be substantially improved with \textit{reliable} priors on LDCs and (2) low-fidelity priors can yield substantial biases on $D$. We ground our injection--retrieval framework with intensity profiles from modern stellar atmosphere models spanning a wide range of $T_{\rm eff}$, $\log g$, $[{\rm M/H}]$, and wavelength.
With current photometric precisions, we recommend fitting transits with both a third-order polynomial law \textit{and} a 4$^{\rm th}$-order non-linear law, as together they provide an optimal compromise between bias and $A$ while  cross-checking one another's fidelity. 
The prevailing practice of using a quadratic limb-darkening law yields low $A\sim3$--$5$ but transit depth biases of $5$--$10\sigma$---potentially affecting atmospheric studies due to the inherent wavelength-dependence. Adopting the recommended laws with reasonable LDC priors (10--20\% width) slightly increases $A$ to ${\sim}5$--$6$, while reducing bias below $2\sigma$. Further improvements to limb-darkening models would reduce $A$ and bias jointly, enabling the same science with fewer transits.
...
To first order, one could expect that $\sigma_D$ relates to the scatter in the photometric time series (\autoref{fig:fig1}, left panel), which includes both photon noise (roughly Gaussian at high count levels) and correlated noise (marginal to first order in many modern datasets), measurable as the out-of-transit scatter for a bin of duration similar to the transit duration ($\sigma_{\rm ref} = \sigma_{\rm OOT}/\sqrt{N}$ with $N$ in-transit points). Specifically, one expects that ${\sigma_D} = \sqrt{3} \ {\sigma_{\rm ref}}$, as the typical out-of-transit baseline for transit observations is twice the duration of the transit event targeted (see \autoref{sec:appendix_sqrt} for a full derivation). However, when comparing these two quantities in many existing datasets, a significant discrepancy between expectation and reality emerges (\textit{i.e.}, ${\sigma_D}/\sigma_{\rm ref}\gg \sqrt{3}$). 
...
\textit{Coefficient extraction.} Each profile is interpolated onto a uniform $100$-point $\mu$-grid via a cubic spline and fit with the fourth-order non-linear LDL\footnote{Throughout this work, all LDL fits to an intensity profile are performed via non-linear least squares optimization using \texttt{lmfit} \citep[][]{lmfit}.}. This approach to intensity profile fitting, first presented by \citealt{Claret2011}, was shown by \citealt{Espinoza2015} (hereafter \citetalias{Espinoza2015}) to minimize the residuals between the true profile and its parametric approximation. In doing so, we reduce the dataset from a ($T_{\rm eff}$ $\log g$, $[\rm M/\rm H]$, $\lambda$)-dependent intensity field to a four-dimensional space of LDC vectors $(c_1, c_2, c_3, c_4)$. The resulting distribution is shown in \autoref{fig:appendix_fig2}, where the clear structure reflects the inherited dependence on the underlying physical parameters (which we make explicit for $T_{\rm eff}$ and wavelength). A detailed exploration of this topology is left to future work.

\begin{figure*}[ht]
    \script{Fig5_plot.py}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Fig5.pdf}
    \caption{\textbf{Representative intensity profiles.} Intensity profiles for the global and cluster modes introduced in \autoref{sec:framework} and used in the analyses of \autoref{sec:amp_factor_cause} and \autoref{sec:boosting_science}. \emph{Top:} A random subset of 100 profiles is shown in each panel's background: for the global mode panel, profiles are colored by their cluster assignment (see \autoref{fig:appendix_fig3}); for each cluster mode panel, profiles belonging to that cluster are shown in gray. The modal intensity profile for each panel is overlaid as a solid colored line, with its fourth-order non-linear LDL fit shown as a dashed black line. The resulting LDCs from these fits, along with the stellar parameters and wavelength used to generate each profile are reported in \autoref{tab:tab1}. \emph{Bottom:} Relative difference residuals between the fourth-order non-linear LDL fit and the true intensity profile. These clearly remain well below the 1\% level, with the exception of the cluster 4 mode where residuals grow towards the low-$\mu$ tail as the intensity profile's behavior becomes increasingly complex.}
    \label{fig:fig5}
\end{figure*}
...
The global mode is used as the injection vector for the analyses in \autoref{sec:amp_factor_cause}. In \autoref{sec:boosting_science}, we inject with the modes of three clusters (clusters 1, 3, and 4; see \autoref{fig:fig5}), testing how our conclusions generalize across the ($T_{\rm eff}$ $\log g$, $[\rm M/\rm H]$, $\lambda$)-space. As shown in \autoref{fig:appendix_fig3}, we select these clusters as they roughly cover the edges and center of the dataset. The LDC vectors and corresponding physical parameters for all four modes are reported in \autoref{tab:tab1}.
...
The model-driven increase in $A$ results from correlations between transit model parameters. Different combinations can generate nearly identical light curves, making it difficult to disentangle each parameter's contribution and recover its true value. A simple way to illustrate this effect is to perturb each parameter by 1$\sigma$ (measured from the posterior output by a MCMC) and examine how the light curve responds, an approach previously used by \citet{deWit2012} to study secondary eclipses. We present the results of this perturbation analysis in \autoref{fig:fig2}, specifically focusing on changes in the transit ingress/egress (top left panel), and center (bottom panel).
...
First, the often-used quadratic (second-order polynomial) LDL is inadequate. Despite achieving a low amplification factor ($A \sim 3$--$6$), it results in substantial transit depth biases for nearly all prior widths tested. The magnitude of this bias scales with the portion of true intensity profile that lies outside the quadratic basis. Concretely, the $c_2$ and $c_4$ terms of the injected fourth-order non-linear LDL are exactly representable as quadratic combinations of $(1-\mu)$, but the half-integer-power terms $c_1(1-\mu^{1/2})$ and $c_3(1-\mu^{3/2})$ are not, and their residuals are precisely what the transit depth must absorb. 
...
Through injection--retrieval simulations, we show that the uncertainty on $D$ is systematically amplified by a factor of $A \gg \sqrt{3}$ while transit depth biases can simultaneously arise from inadequate limb-darkening parameterizations. Crucially, these two effects are not independent: choices that reduce $A$ (tightening LDC priors or lowering the LDL order) often inflate the bias on $D$, and vice versa. We therefore recommend that future studies report the derived $A$ as a sanity check; $A\gg6$ would suggest that tighter constraints are likely accessible, while $A\ll6$ implies that the constraints on $D$ are artificially tight and associated inferences may be biased.
...
Among the LDLs tested, our results disfavor the quadratic law, which introduces large transit depth biases ($\sim 5$--$10\sigma$ depending on the underlying stellar and wavelength regime) despite achieving a low amplification factor. We instead favor the third-order polynomial and fourth-order non-linear LDLs that both provide the best compromise between $A$ and bias. With model-informed LDC priors at the 10--20\% level, both yield $A \sim 5$--$6$ while incurring negligible bias ($\leq2\sigma$). This aligns with previous studies showing that more flexible laws outperform the quadratic law in reproducing stellar intensity profiles (\textit{e.g.}, \citetalias{Espinoza2016}, \citealt{Morello2017}, \citealt{Keers2024}). To verify that these recommendations are not specific to a particular stellar regime, we tested them on representative LDC vectors drawn from a hierarchical clustering of intensity profiles spanning the full ($T_{\rm eff}$, $\log g$, $[{\rm M/H}]$, $\lambda$)-space accessible to modern stellar atmosphere models, and found consistent behavior across the regimes considered.

We also show that additional non-transit observations such as radial velocity monitoring cannot meaningfully mitigate the TDPP because they do not constrain the stellar intensity profile that shapes the transit light curve. Progress will instead come from improved stellar models and novel limb-darkening parameterizations that reduce intra-LDC correlations \citep[\textit{e.g.}, selecting more orthogonal parameter bases, as done by][]{Maxted2018,Maxted2023}. One important subtlety to keep in mind is that tightening priors without improving their accuracy can be counterproductive: when prior centers are biased away from the true LDCs---as is generally the case when fitting a polynomial LDL to a non-linear stellar profile---tighter priors trade reduced $A$ for inflated transit depth bias. The fourth-order non-linear LDL is uniquely immune to this tension, providing additional motivation for its adoption alongside the third-order polynomial. Beyond transit depth, these developments will also benefit other inferences---such as stellar density measurements---that inherit biases from LDC correlations. Finally, the present study focuses on the central transit geometry ($b=0$). Grazing and near-grazing transits introduce qualitatively different parameter degeneracies, and a dedicated follow-up varying the orbital properties of the hypothetical system---including its impact parameter---is needed to determine how our recommendations extend to those regimes.

Overall, our findings demonstrate that the limiting factor in transit depth precision and accuracy is not instrumental but rather our ability to model the stellar photosphere accurately.
Improving the fidelity of stellar atmosphere models, adopting best practices when choosing LDLs, and applying realistic, model-based priors on LDCs can jointly reduce $A$ and the bias---accomplishing the same science with substantially fewer transits. Such advances are necessary to fully exploit the information content of high-quality data from JWST and future observatories.
...
\autoref{fig:appendix_fig2} shows the full distribution of fourth-order non-linear LDCs across $T_{\rm eff}$ and wavelength, \autoref{fig:appendix_fig5} shows the same distribution across $\log g$ and $[{\rm M/H}]$, and \autoref{fig:appendix_fig3} shows the same LDC space partitioned by hierarchical cluster membership.

\begin{figure}[htbp]
    \script{Appendix2_plot.py}
    \centering
    \includegraphics[width=\textwidth]{figures/Appendix2.pdf}
    \caption{\textbf{Distribution of fourth-order non-linear limb-darkening coefficients from the MPS-ATLAS set1 grid.} Double corner plot of the four LDCs ($c_1$, $c_2$, $c_3$, $c_4$) computed over $T_{\rm eff} \in [3500, 9000]$\,K, $\log g \in [3.0, 5.0]$, $[{\rm M/H}] \in [-5.0, 1.5]$, and JWST NIRSpec PRISM wavelengths. The \textit{lower-left} and \textit{upper-right} triangles show LDCs colored by $T_{\rm eff}$ and wavelength, respectively. \textit{Left column:} 1D marginal histograms, grouped by the $T_{\rm eff}$ at which the LDCs were computed. \textit{Right column:} Same as the left column, but with grouping done according to wavelength. An animated version cycling through all four physical dimensions ($T_{\rm eff}$, $\log g$, [M/H], and wavelength) is available on GitHub \href{https://github.com/SamMerc/Transit-Information-Content/blob/main/src/static/corner_animation.gif}{\GitHubIcon}.}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Distribution of fourth-order non-linear limb-darkening coefficients from the MPS-ATLAS set1 grid, by $\log g$ and metallicity.} Same as \autoref{fig:appendix_fig2}, but with the \textit{lower-left} and \textit{upper-right} triangles colored by $\log g$ and $[{\rm M/H}]$, respectively. \textit{Left column:} 1D marginal histograms grouped by $\log g$. \textit{Right column:} Same, grouped by $[{\rm M/H}]$.}
...
\autoref{fig:appendix_fig1} presents the results of our sensitivity analysis.
...
\begin{figure}[htbp]
    ...
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Appendix4.pdf}
