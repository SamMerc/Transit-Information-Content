...
\usepackage{float}
% Allow full-width (figure*/table*) floats to occupy more of a page's
% top before LaTeX defers them to a standalone float page.
...
% Title
\title{What's in Your Transit? A Path to More Science from Exoplanet Transit Data}
...
Yet, as instrumental precision increases, the uncertainty on $D$ drifts from expectations driven solely by \vtwo{photon noise}. 
...
\vtwo{We find $A$ departs substantially from its} theoretical limit due to correlations between $D$\vtwo{, the limb-darkening coefficients (LDCs), and the orbital parameters}. \vtwo{Informing} our analyses with over $500{,}000$ intensity profiles from modern stellar atmosphere models spanning a wide range of $T_{\rm eff}$, $\log g$, $[\rm M/H]$, and wavelength, we show that reliable LDC priors can substantially improve transit-based science, while low-fidelity ones can instead bias $D$.
\vtwo{We isolate each parameter's contribution to $A$ and find the LDCs dominate, changing $A$ by $30$--$50\%$ when they are fixed to truth, against $\sim20\%$ for the inclination.}
The prevailing practice of using a quadratic limb-darkening law yields \vtwo{low $A$ but high transit depth biases}---potentially introducing spurious spectral features affecting atmospheric studies. \vtwo{The severity of these biases depends on where the host star and observing bandpass fall in the space of stellar intensity profiles, and varies non-monotonically with wavelength: a \vtwo{parameterization} adequate for one target, or one wavelength bin, may be inadequate for another.}
As a near-term mitigation, we recommend fitting transits with both a third-order polynomial law \textit{and} a fourth-order non-linear law which optimally balance bias and $A$ while cross-checking one another's fidelity. With reasonable LDC priors (10--20\% width)\vtwo{,} $A$ increases only mildly, while bias drops below $2\sigma$ \vtwo{at the tighter end of that range for all but one of the cases tested}.
\vtwo{Under inverse-square-root scaling with transit count, these priors would already enable the same science with $\sim1$--$3\times$ fewer transits.} The long-term solution lies in improving transit-fitting tools and stellar atmosphere models to reduce $A$ and bias jointly\vtwo{: should future models support $1\%$ prior strengths, the same science would require $\sim4$--$6\times$ fewer transits}.
...
\begin{figure*}[htbp]
    ...
    \caption{ \textbf{Towards optimal limb-darkening \vtwo{parameterization}}. Amplification factor (top panels) and transit depth bias (bottom panels) from our injection--retrieval tests shown for \vtwo{five} representative cluster modes. Results are shown as a function of the number of LDCs (x-axis) under different limb-darkening \vtwo{prior} assumptions, ranging from uniform priors to Gaussian priors of varying widths. Polynomial LDLs are used for cases with 1-9 LDCs, while ``4NLLD" refers to the fourth-order non-linear LDL. Since this is the injection LDL, it yields the best performance.
    Priors informed by current stellar models are highlighted in blue ($10$--$20\%$). As in \autoref{fig:fig1}, the black dashed horizontal line marks $A$'s theoretical limit, and box plots summarize the results from ten MCMC retrievals with different noise realizations. Green vertical bands indicate the regimes considered acceptable: for the bias, results within $2\sigma$ of truth and for the amplification factor, $A \leq 6$.  As discussed in \autoref{sec:impact_prior_choice}, parameterizations meeting both criteria are highlighted with green x-axis labels, and those failing one or both are in red. \vtwo{Clusters 0, 2, and 3 are shown here; clusters 4 and 7 are continued in \autoref{fig:appendix_fig6}.}}
    \label{fig:fig5}
\end{figure*}
...
The limiting factor in transit depth precision and accuracy is not instrumental. Uncertainty in stellar limb darkening propagates into $D$ measurements through parameter correlations, increasing the observational effort needed to reach a given precision. Improved stellar models could reduce this penalty by enabling tight, accurate priors on the LDCs. In our fourth-order non-linear tests, tightening priors from uniform to $1\%$ width reduces transit depth uncertainty by a factor of $2.1$--$2.5$ at fixed data quality. Assuming these uncertainties decrease as the inverse square root of the number of observed transits, this improvement corresponds to reaching the same target $D$ precision with $\sim4$--$6\times$ fewer transits. Realizing these gains requires sufficiently accurate stellar models and limb-darkening prescriptions, rather than simply imposing narrower priors. In the meantime, a dual-LDL strategy remains the most effective way to mitigate the bias incurred by the quadratic law.
...
Consider a transit light curve composed of three equal\vtwo{-}duration segments: a pre-transit baseline, an in-transit segment, and a post-transit baseline, each containing $N$ points. Let $\sigma_{\rm OOT}$ denote the typical per-point photometric uncertainty, assumed identical across all segments and uncorrelated.
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Comparison of LDCs between the MPS-ATLAS set1 and set2 stellar atmosphere models.} \emph{Top four:} Histograms of the signed relative difference $(c_{\rm set2} - c_{\rm set1})\,/\,|c_{\rm set1}|$ for each of the four non-linear LDL coefficients, computed point-by-point across matched grids of $T_{\rm eff}$, $\log g$, $[{\rm M/H}]$, and $\lambda$. Median values (red dashed lines) are near zero for all coefficients, confirming the negligible offset between the two model sets. The IQR grows with coefficient order, from $5\%$ for $c_1$ to $15\%$ for $c_2$ and $18$--$19\%$ for $c_3$ and $c_4$, reflecting the increasing difficulty in constraining higher-order coefficients that capture fine-scale intensity variations. \emph{Bottom four:} Median relative difference as a function of stellar parameters and wavelength, with shaded regions indicating the IQR. The inter-model spread exhibits a clear dependence on the physical parameters considered, underscoring that the IQRs derived from the top panels (and used to motivate the LDC priors in \autoref{sec:constraint_stellar_model}) are summary statistics that do not capture the full breadth of behavior across the parameter space.}
    \label{fig:appendix_fig5}
\end{figure}

\section{\vtwo{Injection--retrieval results for clusters 4 and 7}}\label{sec:appendix_fig6}

\vtwo{For legibility, the injection--retrieval results of \autoref{sec:impact_prior_choice} are split across two figures. \autoref{fig:fig5} presents clusters 0, 2, and 3 in the main text and \autoref{fig:appendix_fig6} presents clusters 4 and 7 in the same format. The two are discussed together in \autoref{sec:impact_prior_choice}.}

\begin{figure*}[htbp]
    \script{Appendix6_plot.py}
    \centering
    \includegraphics[width=0.85\textwidth]{figures/Appendix6.pdf}
    \caption{\textbf{Towards optimal limb-darkening \vtwo{parameterization} (continued).} \vtwo{Amplification factor (top panels) and transit depth bias (bottom panels) for the cluster 4 and cluster 7 injection modes, in the same format and with the same conventions as \autoref{fig:fig5}: results are shown as a function of the number of LDCs under limb-darkening priors ranging from uniform to Gaussian of varying width, with box plots summarizing ten MCMC retrievals with different noise realizations. Green bands mark the regimes considered acceptable ($A \leq 6$; bias within $2\sigma$ of truth). These two clusters are discussed alongside the others in \autoref{sec:impact_prior_choice}}.}
    \label{fig:appendix_fig6}
\end{figure*}

\begin{figure*}[htbp]
    \script{Appendix7_plot.py}
    \centering
    \includegraphics[width=0.85\textwidth]{figures/Appendix7.pdf}
    \caption{Temp}
    \label{fig:appendix_fig7}
\end{figure*}
