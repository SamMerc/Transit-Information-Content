...
The long-term solution lies in improving the stellar atmosphere models and transit-fitting tools to reduce $A$ and bias jointly, thus enabling the same science with up to $\sim5\times$ fewer transits.
...
\autoref{fig:appendix_fig2} and \autoref{fig:appendix_fig5} shows the full distribution of fourth-order non-linear LDCs across $T_{\rm eff}$, $\log g$, $[{\rm M/H}]$, and wavelength, while \autoref{fig:appendix_fig3} shows the same LDC space partitioned by hierarchical cluster membership.
