...
\section{Injection--retrieval results for clusters 4 and 7}\label{sec:appendix_fig5b}

For legibility, the injection--retrieval results of \autoref{sec:impact_prior_choice} are split across two figures. \autoref{fig:fig5a} presents clusters 0, 2, and 3 in the main text and \autoref{fig:fig5b} presents clusters 4 and 7 in the same format. The two are discussed together in \autoref{sec:impact_prior_choice}.

\begin{figure*}[htbp]
    \script{Fig5_plot.py}
    \centering
    \includegraphics[width=0.85\textwidth]{figures/Fig5b.pdf}
    \caption{\textbf{Towards optimal limb-darkening parametrization (continued).} Amplification factor (top panels) and transit depth bias (bottom panels) for the cluster 4 and cluster 7 injection modes, in the same format and with the same conventions as \autoref{fig:fig5a}: results are shown as a function of the number of LDCs under limb-darkening priors ranging from uniform to Gaussian of varying width, with box plots summarizing ten MCMC retrievals with different noise realizations. Green bands mark the regimes considered acceptable ($A \leq 6$; bias within $2\sigma$ of truth). These two clusters are discussed alongside the others in \autoref{sec:impact_prior_choice}.}
    \label{fig:fig5b}
\end{figure*}
