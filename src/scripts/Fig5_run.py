...
#%% Storing outputs
raw_save_dir = '/Volumes/Ajax/Work/PhD/Research/Transit-Information-Content/Fig5_Storage/C0/'
...
if 'PLD' in LDL:
    ...
    for iLDC in range(1,PLD_order+1):
        mod_prop.update(
            {
                f'LD_u{iLDC}' : {'vary':True, 'guess':best_fit_LDCs[iLDC-1], 'bounds':[best_fit_LDCs[iLDC-1] - 0.5, best_fit_LDCs[iLDC-1] + 0.5]},
            }
        )

    #Update plot labels
    fixed_args['labels'] += [f'LD_u{iLDC}' for iLDC in range(1,PLD_order+1)]

    #Update priors
    if 'gauss' in prior_strength:
        #Extract strength
        ...
        for iLDC in range(1,PLD_order+1):
            priors_dic.update(
                {
                    f'LD_u{iLDC}' : {'type':'gauss', 'val':best_fit_LDCs[iLDC-1], 's_val':best_fit_LDCs[iLDC-1] * (strength / 100)},
                }
            )
        ...
        for iLDC in range(1,PLD_order+1):
            priors_dic.update(
                {
                    f'LD_u{iLDC}' : {'type':'uf', 'bounds':[-100, 100]},
                }
            )

    else:
        raise KeyError('Wrong prior strength')
    ...
    #Update plot labels
    fixed_args['labels'] += ['LD_u1','LD_u2','LD_u3','LD_u4']
...
def create_jaxoplanet_model(x, p, fitting):
...
def fit_func_numpyro(x, y, yerr):
    ...
    y_pred = create_jaxoplanet_model(x, p_step_all, True)
...
def emcee_log_probability(p_step, x, y, yerr):
    ...
    #Log-likelihood
    y_pred = create_jaxoplanet_model(x, p, True)
    step_chi2 = jnp.sum( (y_pred - y)**2/yerr**2 )
    lk = -0.5 * (step_chi2 + jnp.sum(jnp.log(2*jnp.pi*yerr**2)))
...
#Check limb-darkening law directory exists
LDL_dir = check_dir(raw_save_dir+f'{LDL}/')
print(f"LIMB DARKENING LAW = {LDL}")

#Check prior strength directory exists
prior_strength_dir = check_dir(LDL_dir+f'{prior_strength}/')
print(f"PRIOR STRENGTH = {prior_strength}")

#Check seed directory exists
fixed_args['save_loc'] = check_dir(prior_strength_dir+f'Seed{seed}/')
print(f"SEED = {seed}")
...
#Pure data
true_lc = create_jaxoplanet_model(init_state_dic['times'], init_state_dic, False)
...
median_lc = create_jaxoplanet_model(init_state_dic['times'], median_params, True)
...
bestfit_lc = create_jaxoplanet_model(init_state_dic['times'], best_params, True)
